<div class="nav-conteiner px-5">
    <nav>
        <div>
             <img id="sino" src="src/images/home/sino.png" alt="Sem-conexion" class="img-home" style="cursor: pointer">
        </div>
            <div>
             <a href="index_home"><img src="src/images/home/emblema.png" alt="Sem-conexion" class="img-home-emblema mt-4"></a>
        </div>
        <div class="menu-btn">
            <i id="menu_celular" class="fa fa-bars fa-2x"></i>
        </div>
        <ul>
            <li>  <a href="index_home"><button class="button-home"><span class="btn-label"><i class="fas fa-home"></i></span>Inicio</button></a> </li>
            <li>  <a href="index_ativar"><button class="button-activate"><span class="btn-label"><i class="fas fa-power-off"></i></span>Ativar</button></a> </li>
            <li>  <a href="index_suporte"><button class="button-support"><span class="btn-label"><i class="fas fa-comment"></i></span>Suporte</button></a> </li>
            <li>  <a href="login"><button class="button-sign">Entrar</button></a> </li>
        </ul>
    </nav>
</div>
